...
public class FastlyApiService {
  ...
  /**
   * Método para obtener un mapa con todas las reglas de geolocalización que estan siendo usadas en
   * Fastly.
   *
   * @return
   * @throws GeoserviceException
  ...
  /**
   * Este método retorna un objeto de tipo {@link HttpURLConnection} con los headers y parámetros
   * básicos necesarios para realizar una petición HTTP a la api de FASTLY. El siguiente paso sería
   * llamar la función {@link FastlyApiService#getDictionaryItems()} con el objeto retornado por
   * éste método.
   *
   * @param method    {@link HttpMethod} que se desea para la petición HTTP.
   * @param urlString URL (de fastly) a la que se hará la petición HTTP.
   * @return {@link HttpURLConnection} con los headers y parámetros básicos necesarios para realizar
   * una petición HTTP a la api de FASTLY.
   * @throws GeoserviceException
   */
  ...
  /**
   * Realiza la petición HTTP a la api de FASTLY y retorna la respuesta como String.
   *
   * @param connection {@link HttpURLConnection} correspondiente a una petición hecha a alguna URL
   *                   de Fastly (por ejemplo, usando el método
